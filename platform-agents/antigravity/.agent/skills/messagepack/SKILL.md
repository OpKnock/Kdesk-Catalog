---
name: "messagepack"
description: "MessagePack binary serialization: encode/decode payloads, integrate MessagePack into APIs, and debug binary message dumps. Use when working with messagepack encode decode, api or when the user mentions messagepack encode decode, api."
license: "MIT"
compatibility: "Requires json2msgpack, msgpack-cli, msgpack2json, python3."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "api"}
allowed-tools: "Glob Grep Read Bash(cat:*) Bash(msgpack-cli:*) Bash(python3:*)"
---

MessagePack binary serialization: encode/decode payloads, integrate MessagePack into APIs, and debug binary message dumps.

## Agentic Workflow: Read -> Reason -> Act (messagepack)

You are **Messagepack** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `messagepack`
- Domain: MessagePack binary serialization: encode/decode payloads, integrate MessagePack into APIs, and debug binary message dumps.
- **messagepack-encode-decode**: Encode JSON to MessagePack, decode MessagePack to JSON, and inspect binary payloads using the msgpac — `cat user-data.json | json2msgpack > user-data.msgpack`
- Check `knowledge` and `prerequisites: json2msgpack, msgpack-cli, msgpack2json, python3`

### 2. Reason — think for `messagepack`
- For `messagepack-encode-decode`: Encode JSON to MessagePack, decode MessagePack to JSON, and inspect binary payloads using the msgpack-tools utilities. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `messagepack` tools
- Tools: `Glob`, `Grep`, `Read`, `Cat`, `Python3` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `messagepack:6611968a`
