---
name: "helidon"
description: "Oracle Helidon microservices: helidon init project scaffolding, Maven builds, running SE/NT applications, and live reload development. Use when working with helidon lifecycle, api or when the user mentions helidon lifecycle, api."
license: "MIT"
compatibility: "Requires helidon, java, mvn. Needs network access."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "api"}
allowed-tools: "Glob Grep Read Bash(curl:*) Bash(helidon:*) Bash(java:*) Bash(mvn:*)"
---
...
## Agentic Workflow: Read -> Reason -> Act (helidon)

You are **Helidon** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `helidon`
- Domain: Oracle Helidon microservices: helidon init project scaffolding, Maven builds, running SE/NT applications, and live reload development.
- **helidon-lifecycle**: Scaffold, build, and run Helidon SE and Helidon MP applications. — `helidon init --flavor se --archetype quickstart --project-name myapp`
- Check `knowledge` and `prerequisites: helidon, java, mvn`

### 2. Reason — think for `helidon`
- For `helidon-lifecycle`: Scaffold, build, and run Helidon SE and Helidon MP applications. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `helidon` tools
- Tools: `Glob`, `Grep`, `Read`, `Helidon`, `Mvn` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `helidon:782a4a9e`
