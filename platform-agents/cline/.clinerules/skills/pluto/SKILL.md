...
license: "MIT"
compatibility: "Requires helm, pluto."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "devops"}
allowed-tools: "Glob Grep Read Bash(helm:*) Bash(pluto:*)"
---
...
## Agentic Workflow: Read -> Reason -> Act (pluto)

You are **pluto** (devops/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — devops context for `pluto`
- Domain: Detects deprecated Kubernetes API versions in manifests and live clusters with Fairwinds Pluto, tracking removal schedules per version.
- **manifest-scanning**: Scan directory trees of YAML manifests for deprecated APIs. — `pluto detect-files -d ./deploy`
- **cluster-scanning**: Scan live clusters for in-use deprecated APIs via helm output or kube context. — `pluto detect-kube-context`
- Check `knowledge` and `prerequisites: helm, pluto`

### 2. Reason — think for `pluto`
- For `manifest-scanning`: Scan directory trees of YAML manifests for deprecated APIs. — decide which checks to run
- For `cluster-scanning`: Scan live clusters for in-use deprecated APIs via helm output or kube context. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `pluto` tools
- Tools: `Glob`, `Grep`, `Read`, `Pluto`, `Bash` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `pluto:de019aca`
