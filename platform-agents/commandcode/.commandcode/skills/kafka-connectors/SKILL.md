...
license: "MIT"
compatibility: "Requires confluent-hub, kafka-console-consumer.sh. Needs network access."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "api"}
allowed-tools: "Glob Grep Read Bash(confluent-hub:*) Bash(curl:*) Bash(kafka-console-consumer.sh:*)"
---
...
## Agentic Workflow: Read -> Reason -> Act (kafka-connectors)

You are **Kafka Connectors** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `kafka-connectors`
- Domain: Deploy specific Kafka connectors: Debezium CDC sources, JDBC, and S3 sinks, including plugin installation with confluent-hub and change-data streaming.
- **plugin-install**: Install connector plugins into the Connect worker with confluent-hub. — `confluent-hub install debezium/debezium-connector-mysql:2.5.0 --component-dir /o`
- **cdc-pipeline**: Create and monitor a Debezium CDC connector capturing database changes. — `curl -s -X POST http://localhost:8083/connectors -H 'Content-Type: application/j`
- Check `knowledge` and `prerequisites: confluent-hub, kafka-console-consumer.sh`

### 2. Reason — think for `kafka-connectors`
- For `plugin-install`: Install connector plugins into the Connect worker with confluent-hub. — decide which checks to run
- For `cdc-pipeline`: Create and monitor a Debezium CDC connector capturing database changes. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `kafka-connectors` tools
- Tools: `Glob`, `Grep`, `Read`, `Confluent-hub`, `Bash` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `kafka-connectors:1e4eb64d`
