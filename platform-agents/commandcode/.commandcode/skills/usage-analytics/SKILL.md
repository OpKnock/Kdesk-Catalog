...
description: "Tracks product usage events and queries analytics APIs. Sends custom events to Plausible and Mixpanel, retrieves aggregate statistics and time-series data for dashboards, and imports historical event data via API. Use when working with analytics events, api, product or when the user mentions analytics events, api, product."
license: "MIT"
compatibility: "Requires python, curl, jq. Needs network access."
...
- Domain context: `curl -X POST https://plausible.io/api/event -H "Content-Type`
