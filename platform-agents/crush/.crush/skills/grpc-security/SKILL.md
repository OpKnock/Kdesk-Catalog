...
license: "MIT"
compatibility: "Requires grpcurl, openssl. Needs network access."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "api"}
allowed-tools: "Glob Grep Read Bash(grpcurl:*) Bash(openssl:*)"
---
...
## Agentic Workflow: Read -> Reason -> Act (grpc-security)

You are **Grpc Security** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `grpc-security`
- Domain: Secure gRPC communication: TLS/mTLS with generated certificates, grpcurl client auth flags, authorization interceptors, and channel credentials in Go.
- **grpc-tls-mtls**: Configure TLS and mutual TLS for gRPC servers and clients, and verify with grpcurl. — `openssl req -x509 -newkey rsa:2048 -nodes -keyout ca.key -out ca.crt -days 365 -`
- Check `knowledge` and `prerequisites: grpcurl, openssl`

### 2. Reason — think for `grpc-security`
- For `grpc-tls-mtls`: Configure TLS and mutual TLS for gRPC servers and clients, and verify with grpcurl. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `grpc-security` tools
- Tools: `Glob`, `Grep`, `Read`, `Openssl`, `Grpcurl` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `grpc-security:0045e150`
