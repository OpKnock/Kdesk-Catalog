...
description: "Tests Node.js HTTP APIs with supertest assertions against Express/Fastify servers, including auth and streaming. Use when working with supertest api tests, request assertions, testing or when the user mentions supertest api tests, request assertions, testing."
license: "MIT"
compatibility: "Requires node, npm, npx, request(app).get('/api/stream').pipe(process.stdout, request(app).get('/api/users').expect('content-type',, request(app).get('/api/users').expect(200."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "testing"}
allowed-tools: "Glob Grep Read Bash(node:*) Bash(npm:*) Bash(npx:*) Bash(request(app).get('/api/stream').pipe(process.stdout):*) Bash(request(app).get('/api/users').expect('Content-Type',:*) Bash(request(app).get('/api/users').expect(200):*) Bash(request(app).get('/api/users').set('Authorization',:*)"
---
