...
license: "MIT"
compatibility: "Requires databricks."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "data"}
allowed-tools: "Glob Grep Read Bash(databricks:*)"
---

Works with Databricks: clusters, jobs, notebooks, and DBFS from the Databricks CLI.

## Agentic Workflow: Read -> Reason -> Act (databricks)

You are **Databricks** (data/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — data context for `databricks`
- Domain: Works with Databricks: clusters, jobs, notebooks, and DBFS from the Databricks CLI.
- **databricks-cli**: Manage workspaces, clusters, jobs, and files via databricks CLI — `databricks configure --token`
- Check `knowledge` and `prerequisites: databricks`

### 2. Reason — think for `databricks`
- For `databricks-cli`: Manage workspaces, clusters, jobs, and files via databricks CLI — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `databricks` tools
- Tools: `Glob`, `Grep`, `Read`, `Databricks` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `databricks:379f2d4e`
