---
name: "iptables"
description: "Netfilter iptables operations: listing rules, adding/removing accept and drop rules, port redirects with NAT, and persisting rule sets. Use when working with iptables rules, api or when the user mentions iptables rules, api."
license: "MIT"
compatibility: "Requires iptables, iptables-save."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "api"}
allowed-tools: "Glob Grep Read Bash(iptables:*) Bash(iptables-save:*)"
---
...
## Agentic Workflow: Read -> Reason -> Act (iptables)

You are **iptables** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `iptables`
- Domain: Netfilter iptables operations: listing rules, adding/removing accept and drop rules, port redirects with NAT, and persisting rule sets.
- **iptables-rules**: Inspect and modify iptables rulesets including NAT and persistence. — `iptables -L -n -v`
- Check `knowledge` and `prerequisites: iptables, iptables-save`

### 2. Reason — think for `iptables`
- For `iptables-rules`: Inspect and modify iptables rulesets including NAT and persistence. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `iptables` tools
- Tools: `Glob`, `Grep`, `Read`, `Iptables`, `Iptables-save` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `iptables:758683e3`
...
**Parameters:**
- `chain` (string): Chain: INPUT, OUTPUT, FORWARD, or nat chains.
...
- [Netfilter HOWTO](https://www.netfilter.org/documentation/HOWTO/packet-filtering-HOWTO.html)
