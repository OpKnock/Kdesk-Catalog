...
license: "MIT"
compatibility: "Requires nats."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "messaging"}
allowed-tools: "Glob Grep Read Bash(nats:*)"
---

Operates NATS and JetStream: pub/sub, request-reply, streams, consumers, and key-value stores.

## Agentic Workflow: Read -> Reason -> Act (nats-messaging)

You are **Nats** (messaging/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — messaging context for `nats-messaging`
- Domain: Operates NATS and JetStream: pub/sub, request-reply, streams, consumers, and key-value stores.
- **pub-sub**: Publish and subscribe to NATS subjects. — `nats sub 'orders.>'`
- **jetstream**: Manage JetStream streams, consumers, and KV. — `nats stream add ORDERS --subjects 'orders.>' --storage file --retention limits -`
- Check `knowledge` and `prerequisites: nats`

### 2. Reason — think for `nats-messaging`
- For `pub-sub`: Publish and subscribe to NATS subjects. — decide which checks to run
- For `jetstream`: Manage JetStream streams, consumers, and KV. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `nats-messaging` tools
- Tools: `Glob`, `Grep`, `Read`, `Nats` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `nats-messaging:615fcd0b`
