...
description: "Firestore realtime updates: subscribe to document and collection snapshots, handle listener lifecycle, and debug sync lag. Use when working with realtime listeners, api or when the user mentions realtime listeners, api."
license: "MIT"
compatibility: "Requires firebase, grep, node."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "api"}
allowed-tools: "Glob Read Bash(firebase:*) Grep Bash(node:*)"
---

Firestore realtime updates: subscribe to document and collection snapshots, handle listener lifecycle, and debug sync lag.

## Agentic Workflow: Read -> Reason -> Act (firestore-realtime)

You are **Firestore Realtime** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `firestore-realtime`
- Domain: Firestore realtime updates: subscribe to document and collection snapshots, handle listener lifecycle, and debug sync lag.
- **realtime-listeners**: Attach, manage, and debug Firestore snapshot listeners. — `node -e "const {onSnapshot,doc}=require('firebase/firestore');onSnapshot(doc(db,`
- Check `knowledge` and `prerequisites: firebase, grep, node`

### 2. Reason — think for `firestore-realtime`
- For `realtime-listeners`: Attach, manage, and debug Firestore snapshot listeners. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `firestore-realtime` tools
- Tools: `Glob`, `Read`, `Bash`, `Grep`, `Firebase` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `firestore-realtime:4f63807a`
