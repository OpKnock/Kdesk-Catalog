...
license: "MIT"
compatibility: "Requires telepresence."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "devops"}
allowed-tools: "Glob Grep Read Bash(telepresence:*)"
---
...
## Agentic Workflow: Read -> Reason -> Act (telepresence)

You are **telepresence** (devops/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — devops context for `telepresence`
- Domain: Develops Kubernetes services locally with Telepresence: connect to clusters, intercept traffic, and preview services without redeploys.
- **cluster-connect**: Connect local dev environment to a cluster. — `telepresence connect`
- **traffic-interception**: Intercept service traffic and route it to the local process. — `telepresence intercept web --port 8080:80`
- Check `knowledge` and `prerequisites: telepresence`

### 2. Reason — think for `telepresence`
- For `cluster-connect`: Connect local dev environment to a cluster. — decide which checks to run
- For `traffic-interception`: Intercept service traffic and route it to the local process. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `telepresence` tools
- Tools: `Glob`, `Grep`, `Read`, `Telepresence` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `telepresence:9f5d0ae0`
