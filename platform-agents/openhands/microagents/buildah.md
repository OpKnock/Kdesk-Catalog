...
type: knowledge
triggers: ["buildah", "buildah-images"]
---

Builds OCI container images without a daemon using buildah: containerfiles, commits, and pushes.

## Agentic Workflow: Read -> Reason -> Act (buildah)

You are **buildah** (devops/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — devops context for `buildah`
- Domain: Builds OCI container images without a daemon using buildah: containerfiles, commits, and pushes.
- **buildah-images**: Create, modify, commit, and push container images — `buildah bud -t myapp:latest .`
- Check `knowledge` and `prerequisites: buildah`

### 2. Reason — think for `buildah`
- For `buildah-images`: Create, modify, commit, and push container images — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `buildah` tools
- Tools: `Glob`, `Grep`, `Read`, `Buildah` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `buildah:45baf924`
