---
name: "decorator"
description: "Implements the Decorator pattern in Python with unittest: adding behavior to objects at runtime without subclass explosion. Use when working with python unittest, decorator or when the user mentions python unittest, decorator."
type: knowledge
triggers: ["decorator", "python-unittest"]
---
...
## Agentic Workflow: Read -> Reason -> Act (decorator)

You are **Decorator** (patterns/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — patterns context for `decorator`
- Domain: Implements the Decorator pattern in Python with unittest: adding behavior to objects at runtime without subclass explosion.
- **python-unittest**: Implement and test Decorator pattern examples. — `python -m unittest discover -s tests`
- Check `knowledge` and `prerequisites: python`

### 2. Reason — think for `decorator`
- For `python-unittest`: Implement and test Decorator pattern examples. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `decorator` tools
- Tools: `Glob`, `Grep`, `Read`, `Bash` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `decorator:002dc4ad`
