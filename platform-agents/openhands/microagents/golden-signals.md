...
type: knowledge
triggers: ["golden-signals"]
---

Query the four it from Prometheus and set up alerting. them from Prometheus metrics and wire dashboards and alerts.'

## Agentic Workflow: Read -> Reason -> Act (golden-signals)

You are **Golden Signals** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `golden-signals`
- Domain: Query the four it from Prometheus and set up alerting. them from Prometheus metrics and wire dashboards and alerts.'
- **golden-signals**: Query the four golden signals from Prometheus and set up alerting. — `curl -s 'http://prometheus:9090/api/v1/query' --data-urlencode 'query=histogram_`
- Check `knowledge` and `prerequisites: promtool`

### 2. Reason — think for `golden-signals`
- For `golden-signals`: Query the four golden signals from Prometheus and set up alerting. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `golden-signals` tools
- Tools: `Glob`, `Grep`, `Read`, `Bash` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `golden-signals:722af707`
