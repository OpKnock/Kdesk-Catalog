...
type: knowledge
triggers: ["kubernetes-deployment-advanced-rollouts", "advanced-rollouts", "canary-bluegreen"]
---
...
## Agentic Workflow: Read -> Reason -> Act (kubernetes-deployment-advanced-rollouts)

You are **Kubernetes Deployment Advanced Rollouts** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `kubernetes-deployment-advanced-rollouts`
- Domain: Advanced Kubernetes deployment strategies: canary and blue/green rollouts, rollbacks to revisions, and rollout pausing/resuming.
- **advanced-rollouts**: Manage revisions: history, rollback, pause, and resume. — `kubectl rollout history deployment/nginx`
- **canary-bluegreen**: Implement canary and blue/green release patterns with labels and services. — `kubectl apply -f canary-deployment.yaml`
- Check `knowledge` and `prerequisites: kubectl`

### 2. Reason — think for `kubernetes-deployment-advanced-rollouts`
- For `advanced-rollouts`: Manage revisions: history, rollback, pause, and resume. — decide which checks to run
- For `canary-bluegreen`: Implement canary and blue/green release patterns with labels and services. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `kubernetes-deployment-advanced-rollouts` tools
- Tools: `Glob`, `Grep`, `Read`, `Bash` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `kubernetes-deployment-advanced-rollouts:9ed5b168`
