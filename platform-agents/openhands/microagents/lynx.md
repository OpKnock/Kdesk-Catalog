...
type: knowledge
triggers: ["lynx", "lynx-tests"]
---

End-to-end and unit testing with the Lynx test framework, writing assertions in plain JavaScript with instant watch mode.

## Agentic Workflow: Read -> Reason -> Act (lynx)

You are **Lynx** (code-quality/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — code-quality context for `lynx`
- Domain: End-to-end and unit testing with the Lynx test framework, writing assertions in plain JavaScript with instant watch mode.
- **lynx-tests**: Run, watch, and snapshot-test with the Lynx framework for browser and Node tests — `npx lynx --watch`
- Check `knowledge` and `prerequisites: npx`

### 2. Reason — think for `lynx`
- For `lynx-tests`: Run, watch, and snapshot-test with the Lynx framework for browser and Node tests — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `lynx` tools
- Tools: `Glob`, `Grep`, `Read`, `Npx` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `lynx:782fea76`
