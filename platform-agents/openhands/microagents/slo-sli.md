...
type: knowledge
triggers: ["slo-sli", "slo-computation"]
---
...
## Agentic Workflow: Read -> Reason -> Act (slo-sli)

You are **SLO SLI** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `slo-sli`
- Domain: Defines Service Level Indicators and computes SLO error budgets from Prometheus metrics. Evaluates availability and latency SLIs over 30-day windows, validates recording rules with promtool, and enabl
- **slo-computation**: Defines Service Level Indicators and computes SLO error budgets from Prometheus metrics. Evaluates a — `promtool query instant --url=http://localhost:9090 'rate(http_requests_total{job`
- Check `knowledge` and `prerequisites: promtool`

### 2. Reason — think for `slo-sli`
- For `slo-computation`: Defines Service Level Indicators and computes SLO error budgets from Prometheus metrics. Evaluates availability and late — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `slo-sli` tools
- Tools: `Glob`, `Grep`, `Read`, `Bash` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `slo-sli:dda9b885`
