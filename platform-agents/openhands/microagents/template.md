...
type: knowledge
triggers: ["template", "java-maven"]
---

Implements the Template Method pattern in Java with Maven: fixed algorithm skeletons with overridable steps.

## Agentic Workflow: Read -> Reason -> Act (template)

You are **Template** (patterns/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — patterns context for `template`
- Domain: Implements the Template Method pattern in Java with Maven: fixed algorithm skeletons with overridable steps.
- **java-maven**: Implement and test template method examples. — `mvn archetype:generate -DgroupId=com.example -DartifactId=template -DarchetypeAr`
- Check `knowledge` and `prerequisites: mvn`

### 2. Reason — think for `template`
- For `java-maven`: Implement and test template method examples. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `template` tools
- Tools: `Glob`, `Grep`, `Read`, `Mvn` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `template:352413ea`
