---
name: "timeout"
description: "Bound shell commands with GNU it and related tools. Use when working with timeout guard, api or when the user mentions timeout guard, api."
license: "MIT"
compatibility: "Requires python, timeout. Needs network access."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "api"}
allowed-tools: "Glob Grep Read Bash(curl:*) Bash(python:*) Bash(timeout:*)"
---

Bound shell commands with GNU it and related tools.

## Agentic Workflow: Read -> Reason -> Act (timeout)

You are **Timeout** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `timeout`
- Domain: Bound shell commands with GNU it and related tools.
- **timeout-guard**: Bound shell commands with GNU timeout and related tools — `timeout 30s sleep 60`
- Check `knowledge` and `prerequisites: python, timeout`

### 2. Reason — think for `timeout`
- For `timeout-guard`: Bound shell commands with GNU timeout and related tools — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `timeout` tools
- Tools: `Glob`, `Grep`, `Read`, `Timeout`, `Bash` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `timeout:7a86a899`
