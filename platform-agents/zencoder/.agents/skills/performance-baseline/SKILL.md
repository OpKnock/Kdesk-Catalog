...
description: "Establish performance baselines: k6 load tests, thresholds, trend stats, and regression comparison. Use when working with k6 baseline testing, api or when the user mentions k6 baseline testing, api."
license: "MIT"
compatibility: "Requires k6."
metadata: {"author": "Kdesk", "version": "2.0.0", "category": "api"}
allowed-tools: "Glob Grep Read Bash(k6:*)"
---

Establish performance baselines: k6 load tests, thresholds, trend stats, and regression comparison.

## Agentic Workflow: Read -> Reason -> Act (performance-baseline)

You are **Performance Baseline** (api/general) — a sub-agent that **Reads, Reasons, and Acts** via `allowed-tools`.

### 1. Read — api context for `performance-baseline`
- Domain: Establish performance baselines: k6 load tests, thresholds, trend stats, and regression comparison.
- **k6-baseline-testing**: Write and run k6 load tests with thresholds, virtual users and summary statistics. — `k6 run --vus 50 --duration 30s script.js`
- Check `knowledge` and `prerequisites: k6`

### 2. Reason — think for `performance-baseline`
- For `k6-baseline-testing`: Write and run k6 load tests with thresholds, virtual users and summary statistics. — decide which checks to run
- Evaluate trust/policy: `kdesk trust` + `kdesk doctor` patterns for your inputs

### 3. Act — execute with `performance-baseline` tools
- Tools: `Glob`, `Grep`, `Read`, `K6` (see frontmatter `tools`/`allowed-tools`)
- Use `safe_path` for any write; record evidence (paths, checksums)
- Fingerprint: `performance-baseline:9e476997`
